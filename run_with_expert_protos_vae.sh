...
ds=Libras
EXP_NAME=libras_expert_protos_vae/with
MAX_RUNS=5
...
for run in $(seq 1 $MAX_RUNS); do
  if [[ ! -e experiments/$EXP_NAME/run-$run/$ds/results.json ]] || [[ "$1" == "--overwrite" ]]; then
    ...
    ./main.py --uea_dataset $ds --experiment_name $EXP_NAME/run-$run --proto_len 0.5 --protos_per_class 2 --epochs 200 --warm_epochs 110 --vae_encoder --target_protos_dir ./target_protos_libras/
  fi
done
